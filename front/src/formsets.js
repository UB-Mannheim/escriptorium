...
export function setupFormSet(form) {
    // delete a row
    form.querySelectorAll('.js-formset-delete').forEach(e => e.addEventListener('mouseup', function(ev) {
        var btn = ev.target;
    ...
    form.addEventListener('change', function(ev) {
        ...
        if (row == rows[rows.length-1] && ev.target.value) {
            let prefix = input.id.split('-').splice(0, input.id.split('-').length - 2).join('-').substr(3);
            let totalInput = form.querySelector('#id_' + prefix + '-TOTAL_FORMS');
