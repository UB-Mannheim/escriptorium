...
3. runs migrations,
4. creates a local `admin` account (credentials written to
   `~/Library/Application Support/eScriptorium/credentials.txt`, mode 600),
5. imports the bundled recognition model *german_print* (public, owned by
   `admin`) if no recognition model exists yet,
6. starts Redis, PostgreSQL, a Celery worker and the web server,
7. opens `http://127.0.0.1:8000/` in the default browser,
8. adds a menu bar (status bar) item for stopping the services.
