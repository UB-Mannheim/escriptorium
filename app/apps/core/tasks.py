...
@shared_task(autoretry_for=(MemoryError,), default_retry_delay=10 * 60)
def align(instance_pk=None, user_pk=None, transcription_pk=None, witness_pk=None, n_gram=4, merge=False, full_doc=True, **kwargs):
    try:
        DocumentPart = apps.get_model('core', 'DocumentPart')
        part = DocumentPart.objects.get(pk=instance_pk)
    except DocumentPart.DoesNotExist:
    ...
    try:
        part.align(transcription_pk, witness_pk, n_gram, merge, full_doc)
    except Exception as e:
        if user:
